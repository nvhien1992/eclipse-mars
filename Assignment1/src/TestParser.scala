...
//    if (args.length == 2) {
      val inputFile = new ANTLRFileStream("testparser.txt");//new ANTLRFileStream(args(0))
      val out = new PrintWriter(new File("outpaser.txt"));//new PrintWriter(new File(args(1)));
      try {
        test(inputFile, out);
      } catch {
